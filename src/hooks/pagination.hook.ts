...
const usePagination = <T>(countPerPage : number) => {
    ...
    //         state : 보여줄 객체 리스트 상태            //
    const [viewList, setviewListt] = useState<T[]>([]);
    //         state : 현재 페이지 번호 상태            //
    ...
    //         state : 전체 세션 상태            //
    const [totalSection, setTotalSectionn] = useState<number>(1);
    ...
    //         effect : total list가 변경될 떄 마다 실행할 작업           //
    useEffect (() =>{
    ...
    //         effect : current page 가 변경될 떄 마다 실행할 작업           //
    useEffect (setView,[currentPage]);

    //         effect : current section 가 변경될 떄 마다 실행할 작업           //
